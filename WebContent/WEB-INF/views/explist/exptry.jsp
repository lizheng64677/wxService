...
<script type="text/javascript">
	...
	function initScroller(){
		...
		scroll.setSearchCondition({"page.currentPage":1,expType:3});
	...
	function initClicker(){
		$("#main").on("click",".li-id",function(){
			window.location.href="<c:url value='/experience/toExpDetaiInfo?expType=3&detailId='/>"+$(this).data("exp_detail_id");
	...
	function createSingleLi(now){
		...
		html += "<h2>"+now.pro_name+"</h2>";
		html += "</div>";
		html += "<h3>["+now.title+"]</h3>";
		...
		html += "</div>";
		html += "<p>提供<font>"+now.pro_num+"</font>份<font class='number'>"+now.exp_num+"</font>人已申请</p>";
