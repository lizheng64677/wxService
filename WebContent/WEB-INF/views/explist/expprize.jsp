...
<script type="text/javascript">
	...
	function initScroller(){
		...
		scroll.setUrl("<c:url value='/expVolved/tryListInfo?cityId=${cityid}'/>");
		scroll.setSearchCondition({"page.currentPage":1,expType:0});
	...
	function initClicker(){
		$("#main").on("click",".li-id",function(){
			window.location.href="<c:url value='/experience/toExpDetaiInfo?expType=0&detailId='/>"+$(this).data("exp_detail_id");
	...
	function createSingleLi(now) {
		...
		html += "<div class='box1'>";
		html += "<li>";
		html += "<a href='javascript:void(0);' class='li-id' data-exp_detail_id='"
				+ now.exp_detail_id + "'>";
		html += "<div class='picBox fLeft'>";
		if(now.is_label ==1 && now.exp_stauts != 3 && now.exp_stauts != 2)
		{
			html += '<p class="label">'+now.label+'</p>';
		}
		html += '<img src='+now.exp_img_url+'>';
		//html += "<div class='dwPic'><img src='<c:url value='/resources/images/web/explist/qiang.png'/>' /></div>";
		...
		html += "</div>";
		html += "<p>提供<font>" + now.pro_num * now.installments
				+ "</font>份&nbsp;&nbsp;累计<font class='number'>" + now.exp_num
				+ "</font>人申请</p>";
