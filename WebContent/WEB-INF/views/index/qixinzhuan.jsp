...
<script type="text/javascript">
	...
	function initScroller(){
		...
		scroll.setUrl("<c:url value='/index/findQixinZhuanList?city_id=${cityid}'/>");
	...
	function initClicker(){
		$("#main").on("click",".li-id",function(){
			window.location.href="<c:url value='/index/toQixinZhuanDetail?expId='/>"+$(this).attr("data-expId");
	...
	function createSingleLi(data){
		...
		if(data.is_label ==1 && data.exp_gold_status!=0)
		{
		...
		tempHtml.push('<li class="boxmenoy"><img src="'+"<c:url value='/resources/images/web/u4.png'/>"+'"><span>'+data.user_share_gold+'~'+data.user_max_gold+'</span>金币收益</li>');
		tempHtml.push('</ul>');
		tempHtml.push('</div>');
		if(data.exp_gold_status==0){
